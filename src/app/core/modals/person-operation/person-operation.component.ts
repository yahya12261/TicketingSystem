...
import { EventEmitter, Input, OnInit, Output, QueryList, ViewChildren } from '@angular/core';
...
export class PersonOperationComponent implements
OnInit{
  myForm!: FormGroup;
  ...
  constructor(private personOperationService : PersonOperationService,private spinner : NgxSpinnerService,private formBuilder: FormBuilder,private personService:PersonService,private serviceService:ServiceService,private toast:ToastService,private governmentService:GovernmentService,private cazaService:CazaService,private townService:TownService){
  }
  ngOnInit(): void {
  ...
  setInvalidInputBorder() {
      ...
      if (this.service?.status==='INVALID') {
 ...
 this.serviceControl = this.formBuilder.control(undefined,Validators.required);
  ...
  if (this.myForm.invalid) {
    ...
    const person :IPerson = {
      ...
      dob : new Date(this.myForm.value.year,this.myForm.value.months-1,this.myForm.value.day),
...
data.person = person;
    ...
    data.person = {id:this.inpPerson.id}
  }
    this.personOperationService.create(data).subscribe(result=>{
      if(result.success){
        this.spinner.hide();
